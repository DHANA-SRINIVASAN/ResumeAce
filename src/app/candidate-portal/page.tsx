  ...
  const handleAnalyzeResume = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.message.includes("GoogleGenerativeAI Error") || err.message.includes("500") || err.message.includes("503") || err.message.toLowerCase().includes("service unavailable") || err.message.toLowerCase().includes("internal server error") || err.message.toLowerCase().includes("overloaded")) {
    ...
    return (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="jobs">
                 {jobRecommendations && jobRecommendations.jobs.length > 0 && <JobRecommendationsDisplay recommendations={jobRecommendations} />}

...
const RecommendedJobSchemaRequired = z.object({
  ...
  applicationLink: z.string().url().describe('A direct application link for the job. This field is mandatory.'),
...
- Job Title (e.g., "Software Engineer"). This field is absolutely mandatory.
- Company Name (e.g., "Tech Solutions Inc."). This field is mandatory.
- Location (e.g., "Chennai, India", "Remote"). This field is mandatory.
- Key Required Skills (a list of 3-5 skills, e.g., ["Java", "Spring Boot", "SQL"]). This field is mandatory.
- Short Job Description (2-3 sentences summarizing the role). This field is mandatory.
- Match Score (a percentage from 0 to 100. This is your initial assessment of skill alignment. Please provide a score even for partial matches; the system will perform further filtering and refinement.). This field is mandatory.
- Direct Job Link (a plausible, valid URL, e.g., https://www.linkedin.com/jobs/view/12345). This field is mandatory and must be a URL.
- Platform (The job portal name, e.g., "LinkedIn", "Naukri", "Indeed", "Glassdoor", "SimplyHired"). This field is mandatory.
...
const jobRecommenderFlow = ai.defineFlow(
  ...
  async (input): Promise<JobRecommenderOutput> => {
    ...
    for (const job of rawOutput.jobs) {
      ...
      try {
        // Ensure the link is a valid URL structure. Zod .url() will do more.
